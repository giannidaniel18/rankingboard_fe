...
export default function MobileBottomNav({ onOpenModal, canRecordMatch = true }: MobileBottomNavProps) {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40">
      <div className="absolute inset-0 bg-white/85 dark:bg-canvas/92 backdrop-blur-xl border-t border-black/[0.06] dark:border-white/[0.06]" />
      ...
      <div className="relative flex items-center justify-around h-16 px-2">
        {leftItems.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex flex-col items-center gap-1 min-w-[60px] py-1 transition-colors duration-150 ${
        ...
        {canRecordMatch && (
          <div className="relative -top-4 flex flex-col items-center gap-1">
            <button
              onClick={onOpenModal}
              className="w-14 h-14 rounded-full bg-brand hover:bg-brand-hover active:bg-brand-active active:scale-95 flex items-center justify-center shadow-xl shadow-brand/40 transition-all duration-150 border-4 border-white dark:border-canvas"
              aria-label={dict.nav.newMatch}
            >
              <Plus className="w-6 h-6 text-black" strokeWidth={2.5} />
            </button>
            <span className="text-[8px] font-semibold tracking-wide uppercase leading-none text-brand whitespace-nowrap">
              {dict.nav.newMatch}
            </span>
          </div>
        )}
