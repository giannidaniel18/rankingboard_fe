      ...
      <button
        ...
        className={`flex flex-col items-center justify-center gap-3 p-5 rounded-xl border-2 transition-all w-full min-h-[110px] ${
          isSelected
            ...
            : 'border-black/[0.09] dark:border-white/[0.09] hover:border-brand/30 bg-black/[0.02] dark:bg-white/[0.02]'
        }`}
      >
        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-base shrink-0 ${
          isSelected ? 'bg-brand text-black' : avatarCls
        }`}>
          {team.name[0]?.toUpperCase()}
        </div>
        <div className="text-center">
          <p className={`text-sm font-bold tracking-[0.04em] transition-colors ${
            isSelected ? 'text-brand' : 'text-tx-primary'
          }`}>
            {team.name}
          </p>
          {team.playerIds.length > 1 && (
            <p className="font-mono text-[10px] text-tx-caption mt-0.5">
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-sm bg-surface border border-black/[0.10] dark:border-white/[0.08] rounded-t-2xl sm:rounded-2xl shadow-2xl shadow-black/80 overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <p className="text-[11px] font-mono text-tx-caption text-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[9px] tracking-[0.2em] text-tx-caption">VS</span>
            <div className="flex-1 h-px bg-black/[0.06] dark:bg-white/[0.06]" />
          </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-brand hover:bg-brand-hover active:bg-brand-active disabled:opacity-40 disabled:cursor-not-allowed text-black text-[11px] font-bold tracking-[0.15em] uppercase transition-all"
          >
            {isLoading
