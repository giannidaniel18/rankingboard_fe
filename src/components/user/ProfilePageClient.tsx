      ...
      <div className="flex min-h-screen bg-canvas">
        <div className="flex-1 flex flex-col min-w-0">
          <main className="flex-1 p-4 md:p-8 flex flex-col items-center justify-center gap-4 text-center">
            <span className="font-mono text-4xl text-tx-caption">◇</span>
            <div className="space-y-1.5">
              <p className="font-heading text-sm font-bold tracking-[0.15em] uppercase text-tx-primary">
                User Not Found
              </p>
              <p className="font-mono text-xs text-tx-caption">
                No player with ID{' '}
                <span className="text-brand">{profileId}</span> exists.
              </p>
            </div>
            <Link
              href="/social"
              className="mt-2 px-4 py-2 rounded-sm border border-black/[0.10] dark:border-white/[0.10] text-[10px] font-semibold uppercase tracking-[0.15em] text-tx-secondary hover:border-brand/40 hover:text-brand transition-colors"
            >
              ← Back to Social
            </Link>
          </main>
          ...
          <div className="max-w-xl w-full space-y-6">
            ...
            <div className="flex items-center gap-3">
              <Link
                ...
                className="text-tx-caption hover:text-tx-primary transition-colors text-base leading-none"
              >
                ←
              </Link>
            ...
            {/* Own-profile banner */}
            {isOwnProfile && (
              <div className="flex items-center gap-3 px-4 py-3 rounded-sm border border-brand/20 bg-brand/[0.05]">
                <span className="font-mono text-[10px] text-brand-text dark:text-brand flex-1">
                  You are viewing your own profile.
                </span>
                <Link
                  href="/profile"
                  className="shrink-0 px-3 py-1 rounded-sm border border-brand/30 text-[10px] font-semibold uppercase tracking-[0.12em] text-brand-text dark:text-brand hover:bg-brand/10 transition-colors"
                >
                  Edit
                </Link>
              </div>
              ...
              <div className="px-6 py-5">
                <div className="flex items-start gap-5">
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    ...
                    <p className="font-mono text-[11px] text-brand-text dark:text-brand">
                      {profileUser.alias}
                    </p>
                    {profileUser.bio && (
              ...
              <div className="bg-surface rounded border border-black/[0.08] dark:border-white/[0.07] overflow-hidden">
                ...
                <div className="px-5 py-5 space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="shrink-0 font-heading text-[10px] font-bold tracking-[0.18em] uppercase text-tx-caption">
                      vs
                    </span>
                    ...
                    <div className="flex-1 flex flex-col items-center gap-1.5 min-w-0">
                      ...
                      <p className="font-medium text-xs text-tx-primary truncate w-full text-center leading-tight">
                        {currentUser.name}{' '}
                        <span className="font-mono text-[9px] text-brand-text dark:text-brand">
                          (You)
                        </span>
                      </p>
                    </div>
                  ...
                  <div className="relative h-1 w-full rounded-full overflow-hidden bg-black/[0.08] dark:bg-white/[0.08]">
                    {h2h.totalMatches > 0 ? (
                      <>
                        ...
                        />
                        <div
                          className="absolute right-0 top-0 bottom-0 bg-live"
                          style={{ width: `${barB}%` }}
                        />
                      </>
                    ) : (
                      <div className="absolute inset-0 bg-black/[0.10] dark:bg-white/[0.10]" />
                    )}
                  </div>
