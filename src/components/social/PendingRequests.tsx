            ...
            {incoming.map(req => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-mono text-primary/80 dark:text-primary/70 truncate">{req.user.alias}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    disabled={isPending}
                    className="px-3 py-1 rounded-sm text-xs font-semibold border border-primary/30 text-primary hover:bg-primary/10 transition-colors disabled:opacity-50"
                  >
                    {dict.decline}
                  </button>
                </div>
        ...
        {sent.length === 0 ? (
          ...
          <ul className="space-y-2">
            {sent.map(req => (
              <li
                key={req.id}
                className="flex items-center gap-3 px-4 py-3 rounded-sm border border-black/[0.06] dark:border-white/[0.10] bg-surface/50"
              >
                <div
                ...
                <span className="text-[10px] font-mono text-primary dark:text-primary border border-primary/30 dark:border-primary/60 px-2 py-0.5 rounded-sm">
                  Pending
                </span>
              </li>
            ))}
